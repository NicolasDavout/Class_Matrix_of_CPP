...
public:
	...
	matrix(int, int);
	matrix(matrix &);
	void match();//Get row and column from memory and assign to members
	void Print_matrix();//Print this matrix on console
	...
	void Row_Mtp(int ROW, int k);//Mutiply a row by k
	void Col_Mtp(int COL, int k);//Mutiply a column by k
